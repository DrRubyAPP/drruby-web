    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-dr-border">
      {items.map((d, i) => (
        <div key={i} className="bg-dr-white p-7">
          <div className="text-[12px] font-semibold tracking-[0.2em] uppercase text-dr-red mb-3">
            {d.eyebrow}
          </div>
          <div className="font-serif text-[18px] font-light text-dr-ink mb-2.5 leading-[1.4]">
            {d.title}
          </div>
          <div className="text-[13px] text-dr-mid leading-[1.7]">{d.desc}</div>
        </div>
      ))}
    </div>
  );
