...
describe("clinic treatments (detail + update)", () => {
  beforeEach(resetDb);
  ...
  it("详情可见", async () => {
    ...
    const tx = await repo.create(me.clinicId, {
      patientUserId: patient.id,
      name: "T",
    });
    const { GET } = await import("./route");
    const res = await GET(bareRequest("GET"), params(tx.id));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.id).toBe(tx.id);
  });

  it("改他人诊所治疗 → 404", async () => {
    ...
    const repo = await import("@/lib/db/repositories/clinic/treatment.repo");
    const tx = await repo.create(a.clinicId, {
      patientUserId: patient.id,
      name: "T",
    });
